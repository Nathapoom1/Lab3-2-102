...
SELECT e.EmployeeID, e.FirstName + ' ' + e.LastName AS EmployeeName,
       c.CustomerID, c.CompanyName AS CustomerCompany
FROM Employees e 
...
SELECT p.ProductName, p.UnitPrice, MONTH(o.OrderDate) AS Month,
...
SELECT s.CompanyName, s.ContactName, s.Phone, s.Fax,
       p.ProductName, p.UnitPrice, SUM(od.Quantity) AS TotalQuantity
FROM Suppliers s 
...
SELECT s.CompanyName AS ShipperName, c.CompanyName AS CustomerName,
...
WHERE ca.CategoryName = 'Condiments'
  AND DATEDIFF(YEAR, e.BirthDate, '1998-12-31') > 60
...
LEFT JOIN Orders o ON e.EmployeeID = o.EmployeeID
  AND CONVERT(date, o.OrderDate) = '1998-06-03'
