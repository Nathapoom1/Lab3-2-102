...
SELECT e.FirstName + ' ' + e.LastName AS EmployeeName, e.Title, DATEDIFF(YEAR, e.HireDate, '1998-12-31') AS YearsOfService,
...
SELECT ca.CategoryName,p.ProductID,p.ProductName,
CAST(SUM(od.UnitPrice * od.Quantity * (1 - od.Discount)) AS DECIMAL(18,2)) AS TotalSales
FROM Orders o JOIN [Order Details] od ON o.OrderID = od.OrderID
              JOIN Products p ON od.ProductID = p.ProductID
              JOIN Categories ca ON p.CategoryID = ca.CategoryID
              JOIN Employees e ON o.EmployeeID = e.EmployeeID
WHERE e.FirstName = 'Nancy' AND e.LastName = 'Davolio'AND o.OrderDate BETWEEN '1997-01-01' AND '1997-03-31'
...
WHERE ca.CategoryName = 'Seafood'AND c.City IN ('London','Cowes')AND s.Country = 'Japan'
GROUP BY c.CompanyName, c.City, ca.CategoryName, s.CompanyName;

-- 16.  แสดงรหัสบริษัทขนส่ง ชื่อบริษัทขนส่ง จำนวนorders ที่ส่ง ค่าขนส่งทั้งหมด  เฉพาะที่ส่งไปประเทศ USA
SELECT s.ShipperID,s.CompanyName AS ShipperName,
